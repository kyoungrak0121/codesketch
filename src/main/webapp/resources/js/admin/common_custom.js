...
function getInsertJqGridData(gID){
	var p = $("#"+gID)[0].p, tempArr=[]  ;

	//insert
	for(var k in p.insertData){
		tempArr.push(p.insertData[k]);
	}
	return tempArr; 
}

function getUpdatetJqGridData(gID){
	var p = $("#"+gID)[0].p, tempArr=[];
	
	// update
	for(var k in p.updateData){
		tempArr.push(p.updateData[k]);
	}
	
	return tempArr; 
}

function getDeleteJqGridData(gID){
	var p = $("#"+gID)[0].p, tempArr=[];
	
	//delete
	for(var k in p.deleteData){
		tempArr.push(p.deleteData[k]); 
	}	
	return tempArr; 
}
...
function getJqGridData(gID){
	...
	uploadData["delete"] = getDeleteJqGridData(gID);
	uploadData["update"] = getUpdatetJqGridData(gID);
	uploadData["insert"] = getInsertJqGridData(gID); 
...
function getSelectedJqGridData(gID){
	...
	$.each( jqGridRowId, function( i, v ){
		tempArr.push($this.getRowData(v));
	});
